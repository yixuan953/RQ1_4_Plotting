...
desired_plot_order = ["soybean", "mainrice", "maize", "winterwheat", "All Crops"]
Categories = ["Irrigation", "N", "P"]
colors = ["#007BB8", "#E41F7E", "#FDD804"] 

input_dir = "/lustre/nobackup/WUR/ESG/zhou111/4_RQ1_Analysis_Results/V5_Statistics/3_Exceedance"
...
for basin in Studyareas:
    ...
    df['Crop type'] = pd.Categorical(df['Crop type'], categories=desired_plot_order, ordered=True)
    df = df.sort_values('Crop type').reset_index(drop=True)
    ...
    ax.barh(y - width, df['P'] * 100, width, label='P', color=colors[2], alpha = 0.65, edgecolor='black')
    ax.barh(y, df['N'] * 100, width, label='N', color=colors[1], alpha = 0.65, edgecolor='black')
    ax.barh(y + width, df['Irrigation'] * 100, width, label='Irrigation', color=colors[0], alpha = 0.65, edgecolor='black')

    # Formatting
    ax.xaxis.tick_top() # Moves the ticks/labels to the top
    ax.xaxis.set_label_position('top') # Moves the "Percentage..." title to the top
    ax.set_yticks(y)
    ax.set_yticklabels(df['Display Name'], fontsize=30)
