...
def GetSimSummary(file_name):
    ...
    N_runoff_Rainfed_exceed = N_runoff_Rainfed - Crit_N_kgperha
    N_runoff_Irrigated_exceed = N_runoff_Irrigated - Crit_N_kgperha
    N_runoff_exceed = N_runoff - Crit_N
    P_runoff_Rainfed_exceed = P_runoff_Rainfed - Crit_P_kgperha
    P_runoff_Irrigated_exceed = P_runoff_Irrigated - Crit_P_kgperha
    P_runoff_exceed = P_runoff - Crit_P
    
    return (mask, N_runoff_Rainfed_exceed, N_runoff_Irrigated_exceed, N_runoff_exceed,
             P_runoff_Rainfed_exceed, P_runoff_Irrigated_exceed, P_runoff_exceed)
...
def plot_boundaries(file_name, data_dir, fig_path, basin):
    ...
    range_keys = [
        "N_Runoff_kg_ha", "N_Runoff_kg_ha", "N_Runoff_ktons",
        "P_Runoff_kg_ha", "P_Runoff_kg_ha", "P_Runoff_ktons"
    ]
    
    cmaps = ['RdPu']*3 + [custom_phosphorus_cmap]*3
...
for basin in Studyareas:
    for crop in InputCrops:
        ...
        data_vars = {
            "N_Runoff_Rainfed_exceed": outputs[1],
            "N_Runoff_Irrigated_exceed": outputs[2],
            "N_Runoff_ktons_exceed": outputs[3],
            "P_Runoff_Rainfed_exceed": outputs[4],
            "P_Runoff_Irrigated_exceed": outputs[5],
            "P_Runoff_ktons_exceed": outputs[6],
