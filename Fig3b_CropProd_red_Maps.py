...
def plot_total_basin_production_map(basin):
    """Calculates and plots a spatial map of the total production change (%)

    summed across ALL target crop types for a single basin.
    """
    # 1. Setup Spatial Boundary
    shp_path = os.path.join(DATA_DIR, "2_shp_StudyArea", basin, f"{basin}.shp")
    gdf_boundary = gpd.read_file(shp_path)
    ...
    # 2. Setup Figure (Single Map Layout)
    row_height = 6.0
    fig_width = row_height * aspect
    fig = plt.figure(figsize=(fig_width, row_height))
    ax = fig.add_subplot(1, 1, 1, projection=ccrs.PlateCarree())

    # 3. Aggregate ALL crops spatially for Baseline and Scenario
    total_baseline_grid = None
    total_scenario_grid = None
    ...
    for crop_name in TARGET_CROPS:
        ...
        if b is not None and s is not None:
            if total_baseline_grid is None:
                total_baseline_grid = b.fillna(0)
                total_scenario_grid = s.fillna(0)
            else:
                total_baseline_grid = total_baseline_grid + b.fillna(0)
                total_scenario_grid = total_scenario_grid + s.fillna(0)

    # 4. Always plot boundary outline
    gdf_boundary.boundary.plot(ax=ax, color='black', linewidth=1.0, zorder=3)

    # 5. Calculate and Plot Spatial Change
    im = None
    if total_baseline_grid is not None and total_scenario_grid is not None:
        # Avoid dividing by zero where there is no historical crop production
        change_grid = (100 * (total_scenario_grid - total_baseline_grid) / total_baseline_grid.where(total_baseline_grid > 0))
        
        # Plot the aggregated crop changes using your custom palette
        im = change_grid.plot(ax=ax, transform=ccrs.PlateCarree(), cmap=prod_cmap, norm=prod_norm, add_colorbar=False, zorder=2)
        
        # Load and apply Low Flow Grey Mask
        lr_path = os.path.join(DATA_DIR, "2_StudyArea", basin, "low_runoff_mask.nc")
        if os.path.exists(lr_path):
            with xr.open_dataset(lr_path) as ds_lr:
                lr_mask = ds_lr["Low_Runoff"].reindex_like(change_grid, method='nearest').where(change_grid.notnull())
                lr_mask.plot(ax=ax, transform=ccrs.PlateCarree(), cmap=grey_cmap, add_colorbar=False, zorder=1, alpha=0.5)

    # 6. Map Extent and Scale Bar
    ax.set_extent([lon_min, lon_max, lat_min, lat_max], crs=ccrs.PlateCarree())
    ax.axis('off')
    ...
    # 7. Add Dedicated Colorbar
    if im:
        cbar_ax = fig.add_axes([0.2, 0.05, 0.6, 0.03])  # Positioned safely below the map scale bar
        cbar = fig.colorbar(im, cax=cbar_ax, orientation='horizontal')
        cbar.set_ticks(prod_bounds)
        cbar.ax.set_xticklabels([str(b) for b in prod_bounds], fontsize=12)
        cbar.outline.set_visible(False)
        cbar.set_label('Production Change (%)', fontsize=12, fontweight='bold')

    # 8. Save output
    out_path = os.path.join(FIG_OUT, f"Total_Production_Change_Map_{basin}.png")
    plt.savefig(out_path, dpi=300, bbox_inches='tight')
    print(f"Saved total production map to: {out_path}")
    plt.close()
...
if __name__ == "__main__":
    for basin in STUDY_AREAS:
        ...
        # 1. Your original plot (Saves: Production_Change_[basin].png)
        print(f"-> Generating multi-crop panel map...")
        plot_basin_production(basin)
        
        # 2. The new total summary map (Saves: Total_Production_Change_Map_[basin].png)
        print(f"-> Generating combined total production map...")
        plot_total_basin_production_map(basin)