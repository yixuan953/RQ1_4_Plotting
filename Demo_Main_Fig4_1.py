...
def plot_data(base_p, red_dict, crit_val, crop_name, basin_name):
    ...
    plt.title(f"Annual N Runoff: {crop_name.replace('total ', '').capitalize()} in {basin_name}", fontsize=12)
    plt.xlabel("Year", fontsize=12)
    plt.ylabel("Total N Runoff (ktons N)", fontsize=12)
    ...
    plt.savefig(f"{output_dir}/{basin_name}_{crop_name}_N_runoff.png", dpi=300)
    plt.close()
...
for basin in Studyarea:
    ...
    for crop in Croptypes:
        ...
        def get_crop_p_runoff(csv_path, area_df, area_col):
            ...
            merged["N_total"] = merged[area_col] * (merged["N_surf"] + merged["N_sub"])
            return merged.groupby("Year")["N_total"].sum()
        ...
        crit_loss_nc = f"/lustre/nobackup/WUR/ESG/zhou111/3_RQ1_Model_Outputs/2_Critical_NP_losses/Method3/{crop_crit_map[crop]}/{basin}_crit_N_runoff_kg.nc"
        with xr.open_dataset(crit_loss_nc) as ds_crit:
            crit_val_total = (ds_crit["Critical_N_runoff"] * mask).sum().values.item()
