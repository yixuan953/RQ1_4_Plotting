...
def plot_data(base_p, red_dict, crop_name, basin_name):
    ...
    plt.title(f"Annual production : {crop_name.replace('total ', '').capitalize()} in {basin_name}", fontsize=12)
    plt.xlabel("Year", fontsize=12)
    plt.ylabel("Total production (ktons)", fontsize=12)
    ...
    plt.savefig(f"{output_dir}/{basin_name}_{crop_name}_crop_prod.png", dpi=300)
    plt.close()
...
for basin in Studyarea:
    ...
    for crop in Croptypes:
        ...
        if basin == "Yangtze" and (crop == "mainrice" or crop == "secondrice"):
            ...
            if crop == "secondrice":
                plot_data(yangtze_rice_data["baseline"], yangtze_rice_data["reduction"], "total rice", "Yangtze")
            continue 

        # Standard Crop Plotting
        plot_data(base_p, current_crop_red_dict, crop, basin)
