...
def plot_butterfly_fertilization(study_areas, input_path, output_path):
    ...
    bar_height = 0.45
    offset = bar_height / 2
    c_inorg, c_manure, p_inorg, p_manure = "#C7DB80", "#8AA624", "#C4E1ED", "#429CC0"
    ...
    for df in all_dfs:
        ...
        for nutrient, ax in [('N', ax_n), ('P', ax_p)]:
            ...
            ax.barh(y_indices - offset, df[c_in], bar_height, color=c_inorg, edgecolor='grey')
            ax.barh(y_indices - offset, df[c_ma], bar_height, left=df[c_in], color=c_manure, edgecolor='grey')
            
            # Plot Post-Red (Bottom bar of the pair)
            ax.barh(y_indices + offset, df[p_in], bar_height, color=p_inorg, edgecolor='grey')
            ax.barh(y_indices + offset, df[p_ma], bar_height, left=df[p_in], color=p_manure, edgecolor='grey')
