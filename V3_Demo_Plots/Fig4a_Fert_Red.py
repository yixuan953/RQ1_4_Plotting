...
# Paths
input_dir = "/lustre/nobackup/WUR/ESG/zhou111/4_RQ1_Analysis_Results/V3_Statistics/4_Red_Fert"
output_dir = "/lustre/nobackup/WUR/ESG/zhou111/4_RQ1_Analysis_Results/V3_Demo_Plots/Fig4_Fert_Red"
...
def plot_butterfly_fertilization(study_areas, input_path, output_path):
    ...
    bar_height = 0.4
    offset = bar_height / 2
    c_inorg, c_manure = '#E49BA6', '#92487A'
    ...
    for df in all_dfs:
        ...
        for nutrient, ax in [('N', ax_n), ('P', ax_p)]:
            ...
            # Plot Current (Top bar of the pair)
            ax.barh(y_indices - offset, df[c_in], bar_height, color=c_inorg, edgecolor='white')
            ax.barh(y_indices - offset, df[c_ma], bar_height, left=df[c_in], color=c_manure, edgecolor='white')
            
            # Plot Post-Red (Bottom bar of the pair)
            ax.barh(y_indices + offset, df[p_in], bar_height, color=c_inorg, edgecolor='white', alpha=0.7)
            ax.barh(y_indices + offset, df[p_ma], bar_height, left=df[p_in], color=c_manure, edgecolor='white', alpha=0.7)
            ...
            for i in range(num_crops):
                ...
                if total_curr > 0:
                    ...
                    ax.text(text_x, y_indices[i] + offset, f'-{reduction:.0f}%', 
                            va='center', fontsize=22, color='#3F4E4F',
    ...
    ax_n.set_xlabel('N Fertilization Rate\n[kg N/ha]', fontsize=30, labelpad=20)
    ax_n.tick_params(axis='x', labelsize=24)
    ...
    ax_p.set_xlabel('P Fertilization Rate\n[kg P/ha]', fontsize=30, labelpad=20)
    ax_p.tick_params(axis='x', labelsize=24)
