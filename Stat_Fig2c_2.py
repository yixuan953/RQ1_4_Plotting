...
# output_dir = "/lustre/nobackup/WUR/ESG/zhou111/4_RQ1_Analysis_Results/V5_Statistics/3_Exceedance"

model_summary_dir = "/lustre/nobackup/WUR/ESG/zhou111/3_RQ1_Model_Outputs/4_Analysis4Plotting/0_Summary/3_Red_fert"
output_dir = "/lustre/nobackup/WUR/ESG/zhou111/4_RQ1_Analysis_Results/V5_Statistics/4_Impacts_NPCrop"
    ...
    for crop in Croptypes:
        ...
        if os.path.exists(file_name) and os.path.exists(low_runoff_path) and os.path.exists(PA_path):
            ...
            if total_ha_sum > 0:
                ...
                ha_exceeded = {
                    "Irrigation": float(Irri_HA.where(w_exc).sum()),
                    "N": float(HA.where(n_exc).sum()),
                    "P": float(HA.where(p_exc).sum()),
                    "Irrigation & N": float(Irri_HA.where(w_exc & n_exc).sum()),
                    "Irrigation & P": float(Irri_HA.where(w_exc & p_exc).sum()),
                    "N & P": float(HA.where(n_exc & p_exc).sum()),
                    "All three": float(Irri_HA.where(w_exc & n_exc & p_exc).sum())
                }
                
                pa_exceeded = {
                    "Irrigation": float(Irri_PA.where(w_exc).sum()),
                    "N": float(Total_PA.where(n_exc).sum()),
                    "P": float(Total_PA.where(p_exc).sum()),
                    "Irrigation & N": float(Irri_PA.where(w_exc & n_exc).sum()),
                    "Irrigation & P": float(Irri_PA.where(w_exc & p_exc).sum()),
                    "N & P": float(Total_PA.where(n_exc & p_exc).sum()),
                    "All three": float(Irri_PA.where(w_exc & n_exc & p_exc).sum())
                }
