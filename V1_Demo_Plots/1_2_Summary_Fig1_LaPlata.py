...
Studyarea =  ["LaPlata"] # ["Indus", "LaPlata", "Yangtze", "Rhine"]
Croptypes =  ["soybean"]# ["winterwheat", "maize", "mainrice", "secondrice", "soybean"]
...
for basin in Studyarea:
    ...
    for crop in Croptypes:
        ...
        Exceedance_irrigation_amount = 100 *(Total_irrigation_amount - Sus_irrigation_amount)/Sus_irrigation_amount             
        ...
        Exceedance_N_runoff = 100 *(N_Runoff - Crit_N_Runoff)/Crit_N_Runoff             
        ...
        Exceedance_P_runoff = 100 *(P_Runoff - Crit_P_Runoff)/Crit_P_Runoff              
        ...
        gs = GridSpec(1, 4, figure=fig1, width_ratios=[1, 1, 1, 0.4])
        ...
        total_str = f"{basin_total_irrigation / 10**6:.2f} × $10^{{6}}$"
        ax3.text(0.0, 0.85,f"Total irrigation amount:\n{total_str} m3",ha="center", va="bottom", fontsize=10)
        ax3.text(0.0, 0.75,f"Unsus:\n{100-sus_irrigation_frac:.1f}%",ha="center", va="bottom", fontsize=10)
        ax3.text(0.0, sus_height-0.3,f"Sustain:\n{sus_irrigation_frac:.1f}%",ha="center", va="bottom", fontsize=10)

        # Map 4 - Exceedance amount
        ax4 = fig1.add_subplot(gs[0, 3], projection=proj)
        cmap_div = plt.cm.RdBu_r
        vabs = max(abs(float(Exceedance_irrigation_amount.min())), abs(float(Exceedance_irrigation_amount.max())))
        norm = TwoSlopeNorm(vmin=-vabs, vcenter=0, vmax=vabs)
        im3 = Exceedance_irrigation_amount.plot(ax=ax4,cmap="PRGn",transform=ccrs.PlateCarree(),add_colorbar=False,vmin = vmin, vmax=vmax)
        im_exc1 = Exceedance_irrigation_amount.plot(ax=ax4, cmap=cmap_div, norm=norm, 
                                add_colorbar=True, transform=proj,
                                cbar_kwargs={'orientation': 'horizontal', 'pad': 0.05, 'label': '%'})

        basin_gdf.to_crs("EPSG:4326").plot(ax=ax4, edgecolor="black", linewidth=1.2, facecolor="none")
        ax4.set_title("Exceedance/Sustainable amount (%)")
        ax4.axis('off')
        ...
        vmin = 0
        vmax = 200
        fig4 = plt.figure(figsize=(12, 7)) # Indus
        ...
        # Add absolute amount text on top
        total_str = f"{basin_total_crop_production/10**6:.2f}" # Transform kg to ktons
